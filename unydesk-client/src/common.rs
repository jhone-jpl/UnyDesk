...
fn get_api_server_(api: String, custom: String) -> String {
    ...
    // UnyDesk: no public cloud fallback — require custom rendezvous / api-server.
    "".to_owned()
}

#[inline]
pub fn is_public(url: &str) -> bool {
    // Treat only the upstream RustDesk public cloud as "public" (disables Pro sync).
    // UnyDesk self-hosted / UnySystems domains are never public.
...
pub fn read_custom_client(config: &str) {
    ...
    // UnySystems Ed25519 verify key for custom.txt (see /keys/custom_client_signing.key.json)
    const KEY: &str = "5h72ajqmWmW7Xxx3zCZ+WcbGKfb1fN/ZWJszUXNSWvE=";
